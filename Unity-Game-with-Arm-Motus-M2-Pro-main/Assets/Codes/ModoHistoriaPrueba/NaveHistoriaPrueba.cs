...
public class NaveHistoriaPrueba : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (inputMode == InputMode.ArmMotus && hideSystemCursorWhenArmMotus)
            SetSystemCursor(false, CursorLockMode.Confined);
    ...
    void Update()
    {
        if (!juegoIniciado && naveActiva && Keyboard.current != null && Keyboard.current.tKey.wasPressedThisFrame)
            IniciarJuego();
        if (inputMode == InputMode.ArmMotus && hasRobotSample)
        {
        ...
        Vector3 cursorWorld =
            (inputMode == InputMode.Mouse) ? GetMouseCursorWorld() : GetArmMotusCursorWorld();
        
        UpdateRobotCursorVisual(cursorWorld);
        // 4) Lógica de inicio por "hover 3s" para AMBOS modos
        if (naveActiva && !juegoIniciado)
        {
            // Si usas robot, normalmente conviene ocultar cursor del SO mientras esperas
            if (inputMode == InputMode.ArmMotus && hideSystemCursorWhenArmMotus)
                SetSystemCursor(false, CursorLockMode.Confined);
            else
                SetSystemCursor(true, CursorLockMode.Confined);

            IntentarIniciar(cursorWorld);

            // Si quieres conservar el atajo T SOLO para pruebas, déjalo.
            // Si ya no lo quieres, borra este bloque.
            if (inputMode == InputMode.ArmMotus && Keyboard.current != null && Keyboard.current.tKey.wasPressedThisFrame)
                IniciarJuego();
        }
     ...
     private void IniciarJuego()
    {
        ...
        if (robotCursorVisual != null && showRobotCursorOnlyBeforeStart)
            robotCursorVisual.gameObject.SetActive(false);
   ...
   // ================== CONFIG DESDE MODO HISTORIA ==================
    ...
    private Vector3 GetMouseCursorWorld()
    {
        ...
        // Antes de iniciar: usa el mouse real (cursor del SO)
        if (!juegoIniciado)
        {
            Vector3 mw = Camera.main.ScreenToWorldPoint(Input.mousePosition);
            mw.z = 0f;
            if (clampToCamera) mw = ClampToCameraBounds(mw, clampPadding);
            return mw;
        }

        // Ya iniciado: tu lógica actual (virtual cursor con delta) para que se sienta “tipo SO”
        if (useVirtualCursor)
        {
            if (!virtualCursorInit)
            {
                Vector3 mw0 = Camera.main.ScreenToWorldPoint(Input.mousePosition);
                mw0.z = 0f;
                virtualCursor = mw0;
    ...
    private Vector3 GetArmMotusCursorWorld()
    {
        return new Vector3(targetPos2D.x, targetPos2D.y, 0f);
    }

    private void UpdateRobotCursorVisual(Vector3 cursorWorld)
    {
        if (robotCursorVisual == null) return;

        bool shouldShow = (inputMode == InputMode.ArmMotus);
        if (showRobotCursorOnlyBeforeStart) shouldShow = shouldShow && !juegoIniciado;

        robotCursorVisual.gameObject.SetActive(shouldShow);

        if (shouldShow)
            robotCursorVisual.position = cursorWorld + cursorVisualOffset;
    }
